...
interface Props {
  ...
  width: number
  height: number
  // Image fixe (import statique, ex. src/assets/fond-avatar.webp) — jamais choisie/remplacée par
  // l'utilisateur, contrairement à DraggableImageField (portrait) : juste une décoration positionnée et
  // dimensionnée par calibrage, la même pour toutes les fiches.
  src: string
  calibrate: boolean
  label: string
...
// Image décorative statique positionnée/dimensionnée par calibrage — même mécanique de glisser/
// redimensionner/réserve que DraggableTextarea (largeur ET hauteur, contrairement à DraggableField qui
// ne redimensionne que la largeur), mais sans contenu ni saisie : juste un <img> fixe.
export default function DraggableStaticImage({
  top, left, width: initWidth, height: initHeight, src,
    ...
    <>
      <img
        ...
        style={{
          ...
          objectFit: 'fill',
          pointerEvents: 'none',
          // Les navigateurs n'impriment pas les images de fond CSS par défaut, mais un vrai <img>
          // s'imprime normalement — pas besoin du print-color-adjust utilisé par DraggableCursorRow
          // (qui, lui, pose l'icône en image de fond CSS).
        }}
      />
