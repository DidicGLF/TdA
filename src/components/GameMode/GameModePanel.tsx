...
export default function GameModePanel({ character, descriptions, onChange, reseau, gererDegatsRecusRef, gererPvActualisesRecuRef, gererNouveauTourRecuRef, drainerReseauEnAttente, onClose, screenWidth }: Props) {
  ...
  // Compagnons actifs du PJ, résolus en stats numériques prêtes à être lancées (même fonction que côté
  // MJ dans CombatTab.tsx/compagnonsDe) — jusqu'ici invisibles du joueur, qui ne pouvait pas les faire
  // agir lui-même (le MJ devait jouer à sa place, signalé comme frustrant).
  const compagnonsActifsResolus = useMemo(() => {
    const brut: (string | null)[] = character.compagnonsActifs ?? []
  ...
  // Portraits pour la vue Combat en cartes (bureau, voir plus bas) : le PJ et ses compagnons ont leur
  // image déjà en local (character.portrait / creature.image, dans le personnage du joueur — aucun
  // réseau nécessaire), contrairement aux créatures ennemies (voir reseau.imagesCibles). Deux appels
  // fixes plutôt qu'un appel de hook dans la boucle de rendu (interdit — un nombre de hooks variable
  // romprait les règles des Hooks) : compagnonsActifs a de toute façon au plus 2 emplacements.
  const pjImageSrc = useImage(character.portrait)
  const compagnonImage0 = useImage(compagnonsActifsResolus[0]?.creature.image)
  const compagnonImage1 = useImage(compagnonsActifsResolus[1]?.creature.image)
  const compagnonImages = [compagnonImage0, compagnonImage1]
  ...
  // "plus visuel" sans reprendre le mécanisme d'expansion du MJ). index sélectionne le bon portrait déjà
  // résolu localement (voir compagnonImages ci-dessus, au plus 2 emplacements).
  const renderCompagnonCardVisuelle = (nom: string, creature: (typeof compagnonsActifsResolus)[number]['creature'], index: number) => {
    const dejaAgi = !compagnonPeutAgir(nom)
    const cibleCompagnon = compagnonsCibleChoisie[nom] ?? ''
    const imageSrc = compagnonImages[index]
    // PV du compagnon, transmis par le MJ (voir 'etat-ciblage'.compagnons dans reseauProtocole.ts) —
    // sans ça, une créature qui blessait le compagnon restait invisible côté joueur (signalé par Didic).
    const etatPv = reseau.compagnonsEtat.find(c => c.nom === nom)
            ...
            <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignContent: 'flex-start' }}>
                  ...
                  {compagnonsActifsResolus.map(({ nom, creature }, i) => renderCompagnonCardVisuelle(nom, creature, i))}
                </div>
              </div>
